...
- **Web Deployment:** Deploy the trained model as a local Flask web app for real-time predictions.
...
1. **Store** — During training, KNN simply memorizes all the training data points.
2. **Calculate** — For a new sample, compute the Squared Euclidean Distance to every training point.
3. **Sort** — Sort all distances from smallest to largest.
4. **Vote** — Pick the `k` nearest neighbors (k=5) and count their class labels.
5. **Decide** — The class with the most votes wins (majority voting).
...
|-------|------|:-:|:-:|
| **Custom KNN** | Built from scratch (NumPy) | **97.75%** | **97.62%** |
| **Scikit-Learn KNN** | Built-in (KNeighborsClassifier) | **97.75%** | **97.62%** |
...
A local Flask web app is included in the `webapp/` folder for real-time predictions.
...
Then open **http://localhost:5000** in your browser, enter 9 cell measurements, and get an instant Benign/Malignant prediction.
...
**Source:** [Wisconsin Breast Cancer Dataset](https://archive.ics.uci.edu/ml/datasets/Breast+Cancer+Wisconsin+(Original)) — UCI Machine Learning Repository
...
- **End-to-End Deployment:** Went from raw data → trained model → deployed web app, completing the full ML lifecycle.
...
- Deploy the web app to a cloud platform for public access.
...
🔗 Connect with me on [LinkedIn](https://www.linkedin.com/in/kirtan-pandya-2036233b2)